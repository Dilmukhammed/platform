/**
 * POST /api/v1/admin/organization-approvals/{approvalId}/approve — Approve an organization.
 */
  ...
  async (request, context, { session }) => {
    try {
      ...
      if (updateError || !updatedOrg) {
        console.error("[admin/organization-approvals/approve] Update error:", updateError);
        return toResponse(
      ...
      console.error("[admin/organization-approvals/approve] Unexpected error:", err);
      return toResponse(
        errorResponse(ErrorCodes.INTERNAL_ERROR, "Failed to approve organization."),
      );
    }
