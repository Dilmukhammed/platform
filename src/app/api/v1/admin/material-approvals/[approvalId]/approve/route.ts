/**
 * POST /api/v1/admin/material-approvals/{approvalId}/approve — Approve a material.
 */
...
export const POST = withAuth(
  async (request, context, { session }) => {
    try {
      ...
      if (updateError) {
        console.error("[admin/material-approvals/approve] Update error:", updateError);
        return toResponse(
          errorResponse(ErrorCodes.INTERNAL_ERROR, "Failed to approve material."),
        );
      }
      ...
      // Approval only activates the material; ownership/scope semantics stay as submitted.
      ...
      if (materialUpdateError) {
        console.error("[admin/material-approvals/approve] Material update error:", materialUpdateError);
        return toResponse(
          errorResponse(ErrorCodes.INTERNAL_ERROR, "Failed to activate approved material."),
        );
      }
      ...
      if (approval.requested_by_platform_user_id) {
        ...
        const materialForNotify = (Array.isArray(rawMaterial) ? rawMaterial[0] : rawMaterial) as Record<string, unknown> | null;
        ...
        const { error: notifyError } = await supabase
          ...
          .insert({
            ...
            payload_json: {
              ...
              approvalId,
              message: "Your material has been approved and is now visible in the school library",
            },
          });

        if (notifyError) {
          console.error("[admin/material-approvals/approve] Error creating notification:", notifyError);
      ...
      console.error("[admin/material-approvals/approve] Unexpected error:", err);
      return toResponse(
        errorResponse(ErrorCodes.INTERNAL_ERROR, "Failed to approve material."),
      );
    }
